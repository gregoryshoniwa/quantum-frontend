  ...
  <div>
    <v-toolbar flat color="yellow darken-3">
      <v-toolbar-title class="white--text"
        >Branch to Teller Float Movement</v-toolbar-title
      >
      <v-divider class="mx-2" inset vertical></v-divider>
      <span class="white--text" style="font-size: 20px"
        >{{ branchtotellerData.branch_name }} branch float is :
        {{ branchtotellerData.iso_code }}
        {{ parseFloat(branchtotellerData.branch_float).toFixed(2) }}</span
      >
      <v-spacer></v-spacer>
    ...
    <v-stepper v-model="e1">
      <v-stepper-header>
        ...
        <v-stepper-step editable :complete="e1 > 2" step="2"
          >Selecting To Float Data</v-stepper-step
        >
      ...
      <v-flex md3 pa-5>
        <v-card color="#385F73" dark hover height="140px">
          <v-card-text class="white--text">
            ...
            {{ branchtotellerData.branch_name }}
          </v-card-text>
        </v-card>
      </v-flex>
...
export default {
  ...
  methods: {
    currencySelected() {
      ...
      this.branchtotellerData.branch_id = this.$session.get("user").userdetails.branch_id;
    ...
    getUserFloatbyCurrency() {
      ...
      Axios.post(
        ...
        {
          col: "currency_id",
          value: this.branchtotellerData.currency_id,
          col2: "branch_id",
          value2: this.branchtotellerData.branch_id,
        },
        { headers: headers }
      ).subscribe(
        (res) => {
    ...
    async getBranchFloatbyCurrency() {
      let headers = {
        "Content-Type": "application/json",
        Authorization: "Bearer " + this.$session.get("user").token,
      };
      // console.log(headers)
      await Axios.post(
        this.nodeApi + "/getBranchFloatByCol2",
        {
          value: this.$session.get("user").userdetails.branch_id,
          value1: this.branchtotellerData.currency_id,
        },
        { headers: headers }
      ).subscribe(
        (res) => {
          this.branchdata = res.data.data.one.recordset;
          this.branchtotellerData.branch_name = this.branchdata[0].name;
          this.branchtotellerData.branch_float = this.branchdata[0].amount;
    ...
    transferFloat() {
      if (!this.branchtotellerData.currency_id) {
      ...
      } else if (this.branchtotellerData.amount > this.branchtotellerData.branch_float) {
        this.$swal.fire({
          type: "error",
          title: "Transfer Failed",
        ...
        Axios.post(this.nodeApi + "/updateBranchToTellerFloat", this.branchtotellerData, {
          headers: headers,
        }).subscribe(
          (res) => {
