<template>
  <div>
    <v-toolbar flat color="yellow darken-3">
            <v-toolbar-title class="white--text">Branch to Treasury Float Movement</v-toolbar-title>
            <v-divider
              class="mx-2"
              inset
              vertical
            ></v-divider>
            <span class="white--text" style="font-size:20px;">{{selectedfrombranch.name}} branch float is : {{selectedfrombranch.symbol}} {{parseFloat(selectedfrombranch.amount).toFixed(2)}}</span>
            <v-spacer></v-spacer>      
          ...
          <v-flex md3>
            <v-text-field
            v-if="e1 == 3"
            v-model="branchtotreasuryData.amount"
          ...
          ></v-text-field>
                  ...
                  <v-stepper v-model="e1">
                    <v-stepper-header>
                      ...
                      <v-stepper-step editable :complete="e1 > 3" step="3">Inputting Transfer Amount</v-stepper-step>
                     ...
                     <v-flex md3 pa-5>
                      ...
                      >
                        ...
                        <v-card-text class="white--text justify-center" >
                          ...
                          {{parseFloat(branchtotreasuryData.amount).toFixed(2)}}  
                        </v-card-text>
...
export default {
  ...
  methods: {
    currencySelected(){
      this.branchtotreasuryData.currency_id = this.selectedcurrency.id
      this.branchtotreasuryData.created_by = this.$session.get('user').userdetails.id
      //console.log(this.selectedfromuser)
      this.getBranchFloatbyCurrency()
      
    },
    frombranchSelected(){
      this.branchtotreasuryData.branch_id = this.selectedfrombranch.branch_id 
      //console.log(this.selectedfromuser)
    },
    ...
    getBranchFloatbyCurrency(){
      let headers = {            
         ...
         // console.log(headers)
          Axios.post(this.nodeApi+'/getBranchFloatByCol',{col:'currency_id',value:this.branchtotreasuryData.currency_id},{headers:headers})
          .subscribe(res =>{
    ...
    transferFloat(){
      if(!this.branchtotreasuryData.currency_id){
        this.$swal.fire({
                type: 'error',
      ...
      else if(!this.branchtotreasuryData.branch_id){
        this.$swal.fire({
                type: 'error',
                title: 'Transfer Failed',
      ...
      else if(!this.branchtotreasuryData.amount){
        this.$swal.fire({
                type: 'error',
                title: 'Transfer Failed',
      ...
      else if(this.branchtotreasuryData.amount <= 0){
        this.$swal.fire({
                type: 'error',
                title: 'Transfer Failed',
      ...
      else if(this.branchtotreasuryData.amount > this.selectedfrombranch.amount){
        this.$swal.fire({
                type: 'error',
                title: 'Transfer Failed',
        ...
        //console.log(this.branchtotreasuryData)
        ...
        let headers = {            
          ...
          Axios.post(this.nodeApi+'/updateBranchToTreasuryFloat',this.branchtotreasuryData,{headers:headers})
          .subscribe(res =>{
            
            if(res.data.data.one.recordset){
             this.selectedcurrency='',
            this.selectedfrombranch='',
            this.selectedtotreasury='',
            this.dialogloader = false
            this.branchtotreasuryData = this.branchtotreasuryDataClear
            this.e1 = 1
              this.$swal.fire({
                type: 'success',
