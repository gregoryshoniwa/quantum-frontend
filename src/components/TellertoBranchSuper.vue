<template>
  <div>
    <v-toolbar flat color="blue darken-4">
            <v-toolbar-title class="white--text">Teller to Branch Float Movement</v-toolbar-title>
            <v-divider
              class="mx-2"
              inset
              vertical
            ></v-divider>
            <span class="white--text" style="font-size:20px;">{{selectedfromuser.first_name}}'s float is : {{selectedfromuser.symbol}} {{parseFloat(selectedfromuser.amount).toFixed(2)}}</span>
            <v-spacer></v-spacer>      
          ...
          <v-flex md3>
            <v-text-field
            v-if="e1 == 4"
            v-model="tellertobranchData.amount"
          ...
          ></v-text-field>
                  ...
                  <v-stepper v-model="e1">
                    <v-stepper-header>
                      ...
                      <v-stepper-step editable :complete="e1 > 2" step="2">Selecting To Float Data</v-stepper-step>

                      <v-divider></v-divider>

                      <v-stepper-step editable :complete="e1 > 3" step="3">Selecting From Float Data</v-stepper-step>
                     ...
                     <v-flex md3 pa-5>
                      ...
                      >
                        ...
                        <v-card-text class="white--text justify-center" >
                          ...
                          {{parseFloat(tellertobranchData.amount).toFixed(2)}}  
                        </v-card-text>
...
export default {
  ...
  methods: {
    currencySelected(){
      this.tellertobranchData.currency_id = this.selectedcurrency.id
      this.tellertobranchData.created_by = this.$session.get('user').userdetails.id
    ...
    fromuserSelected(){
      this.tellertobranchData.user_id = this.selectedfromuser.user_id 
      
      //console.log(this.selectedfromuser)
    },
    tobranchSelected(){
      this.tellertobranchData.branch_id = this.selectedtobranch.branch_id 
    ...
    getUserFloatbyCurrency(){
      let headers = {            
         ...
         // console.log(headers)
          Axios.post(this.nodeApi+'/getUserFloatByCol2',{col:'currency_id',value:this.tellertobranchData.currency_id,col2:'branch_id',value2:this.tellertobranchData.branch_id},{headers:headers})
          .subscribe(res =>{
    ...
    getBranchFloatbyCurrency(){
      let headers = {            
         ...
         // console.log(headers)
          Axios.post(this.nodeApi+'/getBranchFloatByCol',{col:'currency_id',value:this.tellertobranchData.currency_id},{headers:headers})
          .subscribe(res =>{
    ...
    transferFloat(){
      if(!this.tellertobranchData.currency_id){
        this.$swal.fire({
                type: 'error',
      ...
      else if(!this.tellertobranchData.user_id){
        this.$swal.fire({
                type: 'error',
                title: 'Transfer Failed',
      ...
      else if(!this.tellertobranchData.branch_id){
        this.$swal.fire({
                type: 'error',
                title: 'Transfer Failed',
      ...
      else if(!this.tellertobranchData.amount){
        this.$swal.fire({
                type: 'error',
                title: 'Transfer Failed',
      ...
      else if(this.tellertobranchData.amount <= 0){
        this.$swal.fire({
                type: 'error',
                title: 'Transfer Failed',
      ...
      else if(this.tellertobranchData.amount > this.selectedfromuser.amount){
        this.$swal.fire({
                type: 'error',
                title: 'Transfer Failed',
        ...
        let headers = {            
          ...
          Axios.post(this.nodeApi+'/updateTellerToBranchFloat',this.tellertobranchData,{headers:headers})
          .subscribe(res =>{
            ...
            this.tellertobranchData = this.tellertobranchDataClear
            this.e1 = 1
              this.$swal.fire({
                type: 'success',
